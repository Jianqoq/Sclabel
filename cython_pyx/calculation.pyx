...
cpdef float new_width(double angle, int height, int width):
    cdef double x, q
    ...
    if isclose(q, 0):
        q = 0
    return abs(height * x + width * q)


cpdef float new_height(double angle, int height, int width):
    cdef double x, q
    if 0 <= angle <= 90 or 180 <= angle <= 270:
        x = cos(radians(angle))
        q = cos(radians(90 - angle))
    ...
    if isclose(q, 0):
        q = 0
    return abs(height * q + width * x)


cpdef float compare(float num1, float num2, int height, int width):
    if num1 > num2:
        if num2 > width:
...
cpdef str get_validname(str savefiledir, str name, int count, form):
...
def generate_augmentfile(*args):
    cdef int progress, count, quality
    cdef str loc, name, savefiledir
